...
def highway_1d(incoming, out_channels, filter_size, \
    ...
    with tf.compat.v1.variable_scope(scope, default_name = name, values=[incoming]) as scope:
        ...
        H = tf.nn.conv1d(input=net, filters=W, stride=1, padding='SAME') + b
        if batch_norm:
            H = tf.contrib.layers.batch_norm(H)
        H = tf.nn.relu(H)

        T = tf.nn.conv1d(input=net, filters=W_T, stride=1, padding='SAME') + b_T
        if batch_norm:
...
def highway_2d(incoming, out_channels, filter_size, \
    ...
    with tf.compat.v1.variable_scope(scope, default_name = name, values=[incoming]) as scope:
        ...
        H = tf.nn.conv2d(input=net, filters=W, strides=[1,1,1,1], padding='SAME') + b
        if batch_norm:
            H = tf.contrib.layers.batch_norm(H)
        H = tf.nn.relu(H)

        T = tf.nn.conv2d(input=net, filters=W_T, strides=[1,1,1,1], padding='SAME') + b_T
        if batch_norm:
            T = tf.contrib.layers.batch_norm(T)
        T = tf.nn.relu(T)
        C = tf.subtract(1.0, T, name="carry_gate")
